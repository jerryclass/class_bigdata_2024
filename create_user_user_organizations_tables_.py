...
for index, row in data.iterrows():
    ...
    sql_command = f"""
        INSERT INTO {table_name} (pseudo_id, city_code, organization_id, grade, class) 
        VALUES ({pseudo_id}, '{city_code}', {organization_id}, {grade}, {user_class});
    """
    cur.execute(sql_command)
...
cur.execute(
    f"""
        CREATE TABLE users AS (
            SELECT DISTINCT pseudo_id, city_code FROM {table_name}
        );
    """
)
conn.commit()
...
cur.execute(
    f"""
        CREATE TABLE user_organizations AS (
            SELECT DISTINCT pseudo_id, organization_id, grade, class FROM {table_name}
        )
    """
)
conn.commit()
